...
function SettingsForm() {
  ...
  const [isCopied, setCopied] = useClipboard(musicBraille);
  ...
  const onSubmit = (data: SettingsForm) => {
    ...
    fd.append('xmlfile', data.musicxml[0])
    console.log(fd)

    fetch('https://musicxml--webelement7.repl.co/submitform/', {
      method: 'POST',
  ...
  return (
    <div className="flex flex-wrap mt-8">
      <form onSubmit={handleSubmit(onSubmit)} className="pr-4 lg:w-2/12">
        ...
        <div className="mb-2">
          <input type="checkbox" id="bar_fermat" placeholder="bar_fermat" {...register("settings.bar_fermat", {})} />
          <label htmlFor="bar_fermat" className="ml-2">Fermata</label>
        </div>
        <h3 className="text-xl mt-4 mb-2">Note</h3>
        <div className="mb-2">
          <input type="checkbox" id="tuplet" placeholder="tuplet" {...register("settings.tuplet", {})} />
          <label htmlFor="tuplet" className="ml-2">Tuplet</label>
        </div>
        <div className="mb-2">
          <input type="checkbox" id="articulation" placeholder="articulation" {...register("settings.articulation", {})} />
          <label htmlFor="articulation" className="ml-2">Articulation</label>
        </div>
        <div className="mb-2">
          <input type="checkbox" id="accidental" placeholder="accidental" {...register("settings.accidental", {})} />
          <label htmlFor="accidental" className="ml-2">Accidental</label>
        </div>
        <div className="mb-2">
          <input type="checkbox" id="note_fermata" placeholder="note_fermata" {...register("settings.note_fermata", {})} />
          <label htmlFor="note_fermata" className="ml-2">Accidental</label>
        </div>
        <div className="mb-2">
          <input type="checkbox" id="fingering" placeholder="fingering" {...register("settings.fingering", {})} />
          <label htmlFor="fingering" className="ml-2">Fingering</label>
        </div>
        <h3 className="text-xl mt-4 mb-2">Direction</h3>
        <div className="mb-2">
          <input type="checkbox" id="wedge" placeholder="wedge" {...register("settings.wedge", {})} />
          <label htmlFor="wedge" className="ml-2">Wedge</label>
        </div>
        <div className="mb-8">
          <input type="checkbox" id="dynamics" placeholder="dynamics" {...register("settings.dynamics", {})} />
      ...
      <div className="bg-green-900 self-start flex-grow px-4 py-4 rounded-2xl shadow-xl ">
        ...
        <div className="mt-8 flex justify-end">
          <button onClick={setCopied} className="py-2 px-4 bg-white hover:bg-green-50 active:bg-green-800 font-semibold rounded-md shadow-md">
